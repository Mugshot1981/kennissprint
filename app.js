...
const paragraphSelection = document.getElementById("paragraphSelection");
const paragraphTileGrid = document.getElementById("paragraphTileGrid");
const paragraphSelectionSummary = document.getElementById("paragraphSelectionSummary");
...
function getSelectedParagraphIds() {
  return Array.from(selectedParagraphIds);
}

function itemMatchesCurrentSelection(item, selectedChapterIds, selectedParagraphIdsList) {
  if (!selectedChapterIds.includes(item.chapterId)) return false;

  const chapter = activeCourse.chapters.find(
    (candidate) => candidate.id === item.chapterId
  );

  if (!chapter?.paragraphs?.length) return true;

  return selectedParagraphIdsList.includes(item.paragraphId);
}

function getAvailableItems(modeId) {
  const selectedChapterIds = getSelectedChapterIds();
  const selectedParagraphIdsList = getSelectedParagraphIds();
  ...
  if (datasetName === "years") {
    return dataset
      .filter((item) => itemMatchesCurrentSelection(
        item,
        selectedChapterIds,
  ...
  if (datasetName === "terms") {
    return dataset.filter((item) => {
      return itemMatchesCurrentSelection(
        item,
        selectedChapterIds,
        selectedParagraphIdsList
      ) && item.type === "begrip";
  ...
  return dataset.filter((item) => itemMatchesCurrentSelection(
    item,
    selectedChapterIds,
    selectedParagraphIdsList
  ));
...
function getQuestionParts(correctItem, quizMode, currentChapterItems) {
  ...
  let wrongOptionPool = currentChapterItems
    .filter((item) => item.id !== correctItem.id)
    .map((item) => item[answerField])
    .filter((value) => value !== undefined && value !== correctOptionText);

  if (wrongOptionPool.length < 3) {
    const rawDataset = activeCourse.datasets?.[config.dataset] || [];
    const fullDataset = config.dataset === "years"
      ? rawDataset.map((item) => ({
          ...item,
          prompt: item.year,
          answer: item.event
    ...
    const fallbackOptions = fullDataset
      .filter((item) => item.id !== correctItem.id)
      .map((item) => item[answerField])
      .filter((value) => value !== undefined && value !== correctOptionText);

    wrongOptionPool = Array.from(new Set([
      ...wrongOptionPool,
      ...fallbackOptions
    ]));
  }
...
function renderParagraphSelector() {
  if (!paragraphSelection || !paragraphTileGrid || !paragraphSelectionSummary) return;

  const selectedChapterIds = getSelectedChapterIds();
  const visibleParagraphs = activeCourse.chapters
  ...
  Array.from(selectedParagraphIds).forEach((paragraphId) => {
    if (!visibleIds.has(paragraphId)) selectedParagraphIds.delete(paragraphId);
  });

  paragraphTileGrid.innerHTML = "";

  if (visibleParagraphs.length === 0) {
    paragraphSelection.classList.add("hidden");
    paragraphSelectionSummary.textContent = "Nog geen paragrafen geselecteerd.";
    return;
  }

  paragraphSelection.classList.remove("hidden");

  visibleParagraphs.forEach((paragraph, index) => {
    const tile = document.createElement("button");
    tile.type = "button";
    tile.className = "paragraph-tile";
    tile.dataset.paragraphId = paragraph.id;
    tile.setAttribute("aria-pressed", "false");
    tile.textContent = `§${index + 1} ${paragraph.title}`;

    tile.addEventListener("click", () => {
      if (selectedParagraphIds.has(paragraph.id)) {
        selectedParagraphIds.delete(paragraph.id);
      } else {
        selectedParagraphIds.add(paragraph.id);
      }

      const isSelected = selectedParagraphIds.has(paragraph.id);
    ...
    paragraphTileGrid.appendChild(tile);
  });

  const selectedVisibleCount = visibleParagraphs.filter((paragraph) =>
    selectedParagraphIds.has(paragraph.id)
  ).length;

  paragraphSelectionSummary.textContent = selectedVisibleCount === 0
    ? "Nog geen paragrafen geselecteerd."
    : `${selectedVisibleCount} ${selectedVisibleCount === 1 ? "paragraaf" : "paragrafen"} geselecteerd`;
}
...
function updateChapterSelectionSummary() {
  ...
  if (paragraphSelection && !paragraphSelection.classList.contains("hidden")) {
    const selectedCount = getSelectedParagraphIds().length;
    paragraphSelectionSummary.textContent = selectedCount === 0
      ? "Nog geen paragrafen geselecteerd."
      : `${selectedCount} ${selectedCount === 1 ? "paragraaf" : "paragrafen"} geselecteerd`;
  }
}
